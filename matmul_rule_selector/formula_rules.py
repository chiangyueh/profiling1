...
def _base_row_owner_loads(m: int, base_m: int, m_count: int,
                          n_count: int, cores: int) -> List[int]:
    """Exact row work assigned by MatmulBaseBlock::UpdateBasicIndex.

    The BASE tail rule below uses one L2 rectangle, rowOrder=0 and a fully
    aligned N axis.  In that domain the kernel maps round ``j`` on AIC ``c``
    to M index ``(c + j*cores) % m_count``.  The residue sequence has period
    at most ``m_count``; count complete periods instead of iterating over all
    output tiles.
    """
    total = m_count * n_count
    rounds = cd(total, cores)
    remainder = total % cores
    period = m_count // gcd(cores, m_count)
    tail_m = m - (m_count - 1) * base_m
    require(0 < tail_m <= base_m, 'BASE_M_TAIL',
            'the closed-form M partition must cover the logical dimension')
    loads: List[int] = []
    for core in range(cores):
        core_rounds = rounds if remainder == 0 or core < remainder else rounds - 1
        cycle = [
            tail_m if (core + j * cores) % m_count == m_count - 1 else base_m
            for j in range(period)
        ]
        complete, residual = divmod(core_rounds, period)
        loads.append(complete * sum(cycle) + sum(cycle[:residual]))
    require(sum(loads) == m * n_count, 'BASE_OWNER_COVERAGE',
            'closed-form owner accounting must cover every logical M row once per N tile')
    return loads


def solve_base_same_grid_tail(
    s: Shape, h: Hardware = Hardware(), ci: Optional[CompileInfo] = None
) -> Optional[dict]:
    """Rebalance a C220 BASE M tail without creating another output task.

    This is the 20-AIC, shape-and-hardware form of the same-grid part of the
    newer MatMulV3 ``OptimizeLoadBalanceBasicKernel`` rule.  It constructs one
    result directly.  There is no candidate set and no latency/cost score.

    The deliberately narrow domain retains the installed 8.1 BASE dataflow:
    128x256x64 FP16/BF16 NN, no ND2NZ, one full-problem L2 rectangle and the
    original 8/4 double-buffered K pipeline.  Only baseM/singleCoreM change.
    """
    validate_input_domain(s, h, ci)
    if not (
        h.cores == 20 and s.dtype in ('fp16', 'bf16') and
        not s.trans_a and not s.trans_b and not s.bias and
        s.format_a == s.format_b == s.format_out == 'ND' and
        128 < s.m <= 672 and s.m % 16 == 0 and
        s.n >= 256 and s.n % 256 == 0 and
        s.k >= 128 and s.k % 128 == 0
    ):
        return None
    conversion_a, conversion_b = existing_conversion(s, h)
    if conversion_a or conversion_b:
        return None

    original_m, base_n, base_k = 128, 256, 64
    m_count = cd(s.m, original_m)
    n_count = cd(s.n, base_n)
    output_tasks = m_count * n_count
    # Retain the newer source rule's steady-state domain.  Below four waves it
    # uses a different full-wave transform whose extra tasks are not monotonic.
    if output_tasks < 4 * h.cores:
        return None
    balanced_m = up(cd(s.m, m_count), 16)
    if balanced_m >= original_m or cd(s.m, balanced_m) != m_count:
        return None

    # No L2 split/search is needed when the complete logical working set is
    # below the source-visible 100 MiB threshold scaled by physical L2 size.
    whole_bytes = (s.m * s.k * s.d + s.k * s.n * s.d +
                   s.m * s.n * s.dc)
    l2_no_split_limit = h.l2 * 100 // 192
    if whole_bytes > l2_no_split_limit:
        return None

    before_loads = _base_row_owner_loads(
        s.m, original_m, m_count, n_count, h.cores)
    after_loads = _base_row_owner_loads(
        s.m, balanced_m, m_count, n_count, h.cores)
    if max(after_loads) >= max(before_loads):
        return None

    fields = dict(
        baseM=balanced_m, baseN=base_n, baseK=base_k,
        singleCoreM=balanced_m, singleCoreN=base_n, singleCoreK=s.k,
        usedCoreNum=h.cores, stepM=1, stepN=1,
        stepKa=8, stepKb=4, depthA1=16, depthB1=8,
        dbL0A=2, dbL0B=2, dbL0C=1, iterateOrder=0,
    )
    l2 = dict(
        mTile=1, nTile=1, mTileBlock=m_count,
        nTileBlock=n_count, calOrder=0,
    )
    usage = dict(
        L0A_double=2 * balanced_m * base_k * s.d,
        L0B_double=2 * base_n * base_k * s.d,
        L0C=balanced_m * base_n * 4,
        L1_AB=(16 * balanced_m + 8 * base_n) * base_k * s.d,
        bias_reserved=0,
    )
    state = dict(
        result_kind='INDEPENDENT_EXECUTABLE_TILING', input=asdict(s),
        hardware=asdict(h), compile_info=asdict(ci), family='BASE',
        fields=fields, l2=l2, resource_bytes=usage,
        conversion_a=False, conversion_b=False,
        geometry_base=[balanced_m, base_n, base_k],
        logical_MN_grid=[m_count, n_count],
        last_M=s.m - (m_count - 1) * balanced_m,
        last_N=base_n, last_K=s.k,
        core_grid_semantics='MN work items',
        same_grid_tail_balance=dict(
            source_anchor='MatMulV3 OptimizeLoadBalanceBasicKernel same-grid transform',
            original_baseM=original_m, improved_baseM=balanced_m,
            m_count=m_count, n_count=n_count,
            output_tasks_before=output_tasks, output_tasks_after=output_tasks,
            task_waves_before=cd(output_tasks, h.cores),
            task_waves_after=cd(output_tasks, h.cores),
            critical_M_rows_before=max(before_loads),
            critical_M_rows_after=max(after_loads),
            minimum_M_rows_before=min(before_loads),
            minimum_M_rows_after=min(after_loads),
            total_M_rows_before=sum(before_loads),
            total_M_rows_after=sum(after_loads),
            per_core_M_rows_before=before_loads,
            per_core_M_rows_after=after_loads,
            critical_MNK_before=max(before_loads) * base_n * s.k,
            critical_MNK_after=max(after_loads) * base_n * s.k,
            full_problem_bytes=whole_bytes,
            l2_no_split_limit=l2_no_split_limit,
        ),
        selection_contract=dict(
            complete_tilings_constructed=1, candidate_enumeration=False,
            pareto_pruning=False, latency_or_cost_score=False,
            history_lookup=False, runtime_tiling_bank=False,
            decision='closed_form_same_grid_tail_balance_then_exact_kernel_owner_inequality',
        ),
    )
