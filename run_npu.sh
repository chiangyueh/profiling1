...
export MATMUL_TARGET_PASSES=200
...
runner="${build_dir}/test_sequential_base_v1"
...
MATMUL_CAMPAIGN=RECTANGULAR_CUBE "${runner}" "${rectangular_workloads[@]}"
MATMUL_CAMPAIGN=REUSE_DIRECTED "${runner}" "${reuse_workloads[@]}"
edge_binary="$(build_edge_kernel)" || {
    cat "${build_log}" >&2
    exit 1
}
MATMUL_CAMPAIGN=CUBE_VECTOR_EDGE MATMUL_EDGE_BINARY="${edge_binary}" \
    "${runner}" "${edge_workloads[@]}"
